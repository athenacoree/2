...
from care_clear_crew.packager import create_downloadable_zip, generate_appeal_pdf
from care_clear_crew.history_db import save_request, increment_case_count, log_activity, check_usage_allowed, save_or_update_pattern
from care_clear_crew.analysis_runner import run_analysis_with_retry
from care_clear_crew.ui.components import get_stepper, display_analysis, display_precheck
...
def render_new_analysis(user: dict):
    ...
    if up_files and st.button("Iniciar Pipeline de Análisis Autónomo", use_container_width=True):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                agent = CareClearCrew(knowledge_files=paths, insurer_name=ins_name)
