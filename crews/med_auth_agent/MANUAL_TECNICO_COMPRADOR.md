# Manual Técnico para el Comprador — MedAuthAgent Multi-Usuario y Multi-Tenant

Este manual detalla la arquitectura de autenticación, identidad, control de roles (RBAC), aislamiento multi-tenant e integridad criptográfica de **MedAuthAgent**. Asimismo, explica los pasos para el despliegue en producción utilizando PostgreSQL en Render.

---

## 1. Arquitectura de Usuarios y Roles (RBAC)

El sistema ahora cuenta con un modelo robusto de usuarios y control de accesos basado en roles (RBAC) implementado en `src/med_auth_agent/auth.py` y persistido en `src/med_auth_agent/history_db.py`.
...
Si la variable de entorno `DATABASE_URL` no está definida, el sistema inicializa una base SQLite local (`med_auth_history.db`). Esto es ideal para pruebas locales o desarrollo rápido de manera autónoma.
...
Si has recolectado datos previos en desarrollo con SQLite (`med_auth_history.db`) y deseas migrarlos a tu nueva base PostgreSQL de producción, utiliza el script proporcionado:
...
python src/med_auth_agent/migrate_to_postgres.py
```
